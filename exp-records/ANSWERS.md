...
**结论**：RMSNorm 对深层 Transformer 的训练稳定性**不可或缺**；它通过约束激活/梯度尺度，大幅扩展了可稳定训练的学习率上界。若将学习率降至 O(1e-4) 量级，模型可勉强训练，但收敛更慢、最终 loss 明显劣于带 norm 的对照——说明归一化的价值不在于「能不能训」，而在于「在多大的 lr 与多深的网络下仍稳定」。
