#!/bin/bash
# §7.3 post-norm 消融：把 RMSNorm 从子层前移到残差相加后（--no-prenorm），对照组复用 lr_best
uv run python -m cs336_basics.train \
    ...
    --checkpoint-dir /root/autodl-tmp/cs336/data/checkpoints/postnorm \
    --wandb-project cs336-assignment1 --wandb-run-name postnorm \
    --no-prenorm